...
Delegate voting rights in Annex Governance using an EIP-712 signature.

* `_address` \(string\) The address to delegate the user's voting rights to.
* `nonce` \(number\) The contract state required to match the signature. This can be retrieved from the ANN contract's public nonces mapping.
...
(async function() {
  const delegateTx = await annex.delegateBySig(
